window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BillingSection(props) {
      ...
      var balanceRows = providers.map(function (p) {
        var b = p.balance;
        var text, color;
        if (!b) {
          text = "未配置余额端点";
        ...
        return el("div", { key: p.provider, style: { padding: "8px 0", borderBottom: "1px solid rgba(128,128,128,0.14)" } },
          el("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline" } },
            el("span", { style: { fontSize: 12, color: "var(--dsw-alias-label-secondary, #8a8a8a)" } }, p.label || p.provider),
            el("span", { style: { fontSize: 13, fontWeight: 600, color: color, fontVariantNumeric: "tabular-nums" } }, text)
          ),
          b && el("div", { style: { marginTop: 3, fontSize: 11, color: "var(--dsw-alias-label-caption, #666)" } },
      ...
      return el(React.Fragment, null,
        el(Section, { title: "账户余额" },
          balanceRows.length === 0
            ? el("div", { style: { padding: "8px 0", fontSize: 12, color: "var(--dsw-alias-label-caption, #666)" } }, "未发现任何 provider。")
            : balanceRows,
