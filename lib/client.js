window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // -------------------------------------------------------------------------
    // Home-page (no-session) floating entry: rendered through shell.overlay.
    // Hides itself while a session is current, where the header utilities seat
    // already shows the same control next to the Session log.
    // -------------------------------------------------------------------------

    function TideGaugeHomeButton(props) {
      // Hooks must be called unconditionally before any early return: this entry
      // stays mounted in shell.overlay while the current session flips between
      // blank and real, so a conditional hook count would crash React and
      // abdicate the entry (button disappears until a full reload).
      var openState = React.useState(false);
      var open = openState[0];
      var setOpen = openState[1];

      var current = props.useSessions ? props.useSessions(function (s) {
        var id = s && s.current;
        if (id === undefined) return undefined;
        var row = s.byId ? s.byId[id] : undefined;
        return row && row.blank === false ? id : undefined;
      }) : undefined;
      ...
      var panel = open ? el("div", {
        ...
        el(BillingSection, { tokenUsage: null }),
        el("div", {
          style: {
            marginTop: 12,
            fontSize: 11,
          ...
          "本会话的 token 用量、上下文占用与费用估算需进入会话后查看。")
      ) : null;
